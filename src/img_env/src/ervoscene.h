...
class ERVOScene:public Scene{
    public:
        RVO::ERVOSimulator *rvo_sim;
        int ped_num_;
        ERVOScene(map<string, double> params_double, map<string, vector<double>> params_vecdouble):Scene(params_double, params_vecdouble){
            rvo_sim = new RVO::ERVOSimulator();
        ...
        void addRobot(int robo_num){
            ...
            if(relation_ped_robo==1)
                rvo_sim->addAgent(RVO::Vector2(0, 0), 0.5, 10, 5, 5, 0.5, 0.6);
            };
