...
public:
    ...
    // 在grid_map上绘制bbox形状
    bool draw(GridMap& grid_map, int value, string target_frame, vector<Point2d>& bbox);
    bool draw_rgb(GridMap &grid_map, Vec3b value, string target_frame, vector<Point2d>& bbox);
    // 在grid_map上截取机器人视角图
