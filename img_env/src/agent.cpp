...
Agent::Agent(string ktype, double resolution)
{
    ktype_ = ktype;
...
bool Agent::draw(GridMap &grid_map, int value, string target_frame, vector<Point2d>& bbox)
{
...
bool Agent::draw_rgb(GridMap &grid_map, Vec3b value, string target_frame, vector<Point2d>& bbox)
{
    tf::Transform tf_base_world = get_base_world();
    for (vector<Point2d>::iterator it = bbox.begin(); it != bbox.end(); it++)
    {
...
{
    if ((p.x - robot_pose_.x) * (p.x - robot_pose_.x) + (p.y - robot_pose_.y) * (p.y - robot_pose_.y) < p.z * p.z)
        return true;
    else
        return false;
}
